...
if submitted:
    ...
    if film_name != '':
        ...
        # liste_genres_bonus_film = []
        # for genre in liste_genres_film :
        #     if genre in liste_genres_bonus :
        #         liste_genres_bonus_film.append(genre)
        #         liste_genres_film.remove(genre)

        #posiciones_genres = [X.columns.get_loc(col) for col in liste_genres_film]
        #posiciones_genres_bonus = [X.columns.get_loc(col) for col in liste_genres_bonus_film]

        #posiciones_startYear = X.columns.get_loc('startYear')
        ...
        X_scaled[:, positions_others] *= (-5)
        X_scaled[:, posiciones_genres] *= 10
        X_scaled[:, posiciones_actors] *= 3
        X_scaled[:, posiciones_director] *= 2
        #X_scaled[:, posiciones_startYear] *= 1.5
        X_scaled[:, X.columns.get_loc('NL')] *= 4
        X_scaled[:, X.columns.get_loc('title_similary')] *= 7
